...
class PostsListItem extends React.Component {
  render() {
    const { title, authorName, authorEmail, excerpt, slug, date, language, tags } = this.props
        ...
        <footer>
          {authorName &&
            <Author>
              ...
              {authorName && authorEmail &&
                <>Von: <a href={`mailto:${authorEmail}`}>{authorName}</a></>
              }
            </Author>
          }
          <TagList tags={tags} icon={true} />
          <PostDate>{date}</PostDate>
          <ReadPost to={slug}>Mehr lesen ›</ReadPost>
        </footer>
      </Post>
    )
  }
